    ...
    public class TileController : Controller
    {
        ...
        public ActionResult Tile(string service, int? x, int? y, byte? z)
        {
        	...
        	if (tilecannon.SqLiteConnections.ContainsKey(service)){
        		...
        		var file = GetTile(service, x, y, z);
				...
				var mbtiles = Directory.GetFiles(tilecannon.TILEPATH);

				foreach (var newfile in mbtiles){

	            	if (Path.GetFileNameWithoutExtension(newfile) == service){

		                tilecannon.SqLiteConnections.Add(Path.GetFileNameWithoutExtension(newfile), new SQLiteConnection(@"Data Source="+newfile));

		                var tile = GetTile(service, x, y, z);
